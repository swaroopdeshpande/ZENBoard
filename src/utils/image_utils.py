...
def stem_darken(img, threshold=200):
    """Stem darkening for e-ink text: snaps anti-aliased glyph-edge pixels
    below the threshold to pure black, compensating for how e-ink displays
    lose thin/light strokes after BWR quantization. Only affects near-gray
    pixels (font edges) - pure colors above the threshold (white bg, red
    accents with a high R channel) pass through unchanged. Real technique
    used by FreeType/e-reader font engines, adapted here as a post-render
    curve since we do not control glyph hinting in headless Chromium."""
    img = img.convert("RGB")
    lut = [0 if i < threshold else i for i in range(256)] * 3
